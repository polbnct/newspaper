...
if (isset($_POST['insertArticleBtn'])) {
    ...
    if ($articleObj->createArticle($title, $description, $author_id, $category_id, $imagePath, 0)) {
        header("Location: ../index.php");
    } else {
...
if (isset($_POST['editArticleBtn'])) {
	...
	if ($articleObj->updateArticle($article_id, $title, $description, $category_id)) {
        // After updating, re-lock the article and mark as pending review again.
        $articleObj->setEditableStatus($article_id, 0);
        $articleObj->updateArticleVisibility($article_id, 0);
		header("Location: ../index.php");
	}
}
